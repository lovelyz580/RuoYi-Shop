...
public class HttpUtils {
		...
		try {
			...
			if (url != null) {
				...
				if (data != null && !TextUtils.isEmpty(data)) {
					...
					output.write(data.getBytes(OAwechatConfig.CHARACTER_ENCODING));
					output.flush();
					output.close();
				}
	...
	public static InputStream toInputStream(String inputStr) throws IOException {
		...
		return IOUtils.toInputStream(inputStr, OAwechatConfig.CHARACTER_ENCODING); // IOUtils.toInputStream(inputStr);//不写好像是默认UTF-8
	}
